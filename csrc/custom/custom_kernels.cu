...
__global__ void wvSpltK_hf_m1_(const int K, const int N, const DTYPE* B, const DTYPE* __restrict__ A, DTYPE* C, const int CuCount) {
    ...
    //----------------------------------------------------
    for (uint32_t k = 0; k < min(K * M, 32*1024); k += THRDS * WvPrGrp * A_CHUNK)
    {
        uint32_t  k_in = k + ((threadIdx.y * THRDS + threadIdx.x) * A_CHUNK);
	    
        // Transpose of A implementation
        //uint32_t k_ot = (k_in / M) + (k_in % M) * K; // transopse for bank-conflict-free readback
...
/*
#if (YTILE >= 9) 
                if (n+8>=N) continue; bigB8[k2].h8 = (loadnt((half8*)(&B_[8 * K])));
#endif
#if (YTILE >= 10) 
                if (n+9>=N) continue; bigB9[k2].h8 = (loadnt((half8*)(&B_[9 * K])));
#endif
#if (YTILE >= 11) 
                if (n+10>=N) continue; bigB10[k2].h8 = (loadnt((half8*)(&B_[10 * K])));
#endif 
*/
	        }
...
#pragma unroll 
                    ...
                    { 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][0]) : "0"(sum[m][0]), "v" (bigA[m][k2].f[b]), "v" (bigB0[k2].f[b]));

                        //----------------------------------------------------
                        // The following code with YTILE > 1 
                        //----------------------------------------------------
#if (YTILE >= 2) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][1]) : "0"(sum[m][1]), "v" (bigA[m][k2].f[b]), "v" (bigB1[k2].f[b]));
#endif
#if (YTILE >= 3) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][2]) : "0"(sum[m][2]), "v" (bigA[m][k2].f[b]), "v" (bigB2[k2].f[b]));
#endif
#if (YTILE >= 4) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][3]) : "0"(sum[m][3]), "v" (bigA[m][k2].f[b]), "v" (bigB3[k2].f[b]));
#endif
#if (YTILE >= 5) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][4]) : "0"(sum[m][4]), "v" (bigA[m][k2].f[b]), "v" (bigB4[k2].f[b]));
#endif
#if (YTILE >= 6) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][5]) : "0"(sum[m][5]), "v" (bigA[m][k2].f[b]), "v" (bigB5[k2].f[b]));
#endif
#if (YTILE >= 7) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][6]) : "0"(sum[m][6]), "v" (bigA[m][k2].f[b]), "v" (bigB6[k2].f[b]));
#endif
#if (YTILE >= 8) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][7]) : "0"(sum[m][7]), "v" (bigA[m][k2].f[b]), "v" (bigB7[k2].f[b]));
#endif
#if (YTILE >= 9) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][8]) : "0"(sum[m][8]), "v" (bigA[m][k2].f[b]), "v" (bigB8[k2].f[b]));
#endif
#if (YTILE >= 10) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][9]) : "0"(sum[m][9]), "v" (bigA[m][k2].f[b]), "v" (bigB9[k2].f[b]));
#endif
#if (YTILE >= 11) 
                        asm ("v_dot2c_f32_f16 %0, %2, %3" : "=v" (sum[m][10]) : "0"(sum[m][10]), "v" (bigA[m][k2].f[b]), "v" (bigB10[k2].f[b]));
#endif
                    }
                }
	        }
        ...
        for (int m = 0; m < M; m++) 
        {
            for (int y = 0; y < YTILE; y++) 
            {
                ...
                asm("s_nop 0\n\tv_add_f32 %0, %2, %3 row_shl:8 bound_ctrl:0 " : "=v" (sum[m][y]) : "0"(sum[m][y]), "v" (sum[m][y]), "v" (sum[m][y]));
                asm("s_nop 0\n\tv_add_f32 %0, %2, %3 row_shl:4 bound_ctrl:0 " : "=v" (sum[m][y]) : "0"(sum[m][y]), "v" (sum[m][y]), "v" (sum[m][y]));
                asm("s_nop 0\n\tv_add_f32 %0, %2, %3 row_shl:2 bound_ctrl:0 " : "=v" (sum[m][y]) : "0"(sum[m][y]), "v" (sum[m][y]), "v" (sum[m][y]));
                asm("s_nop 0\n\tv_add_f32 %0, %2, %3 wave_shl:1 bound_ctrl:0" : "=v" (sum[m][y]) : "0"(sum[m][y]), "v" (sum[m][y]), "v" (sum[m][y]));
...
__global__ void wvSpltK_hf_m2_(const int K, const int N, const DTYPE* B, const DTYPE* __restrict__ A, DTYPE* C, const int CuCount) {
...
__global__ void wvSpltK_hf_m3_(const int K, const int N, const DTYPE* B, const DTYPE* __restrict__ A, DTYPE* C, const int CuCount) {
...
__global__ void wvSpltK_hf_m4_(const int K, const int N, const DTYPE* B, const DTYPE* __restrict__ A, DTYPE* C, const int CuCount) {
...
void wvSpltK_(void *in_a, void *in_b, void *out_c, const int M_in, const int K_in,const int N_in, cudaStream_t stream, const int CuCount = 0) {
    dim3 grid(CuCount);
    ...
    switch(N_in) {
        case 1:
            wvSpltK_hf_m1_<<<grid, block, 0, stream>>>(K_in, M_in, af4, bf4, c, CuCount);
            break;
        case 2:
            wvSpltK_hf_m2_<<<grid, block, 0, stream>>>(K_in, M_in, af4, bf4, c, CuCount);
            break;
        case 3:
	    wvSpltK_hf_m3_<<<grid, block, 0, stream>>>(K_in, M_in, af4, bf4, c, CuCount);
            break;
        case 4:
            wvSpltK_hf_m4_<<<grid, block, 0, stream>>>(K_in, M_in, af4, bf4, c, CuCount);
            break;
        default:
            throw std::runtime_error("Unsupported N value: " + std::to_string(M_in) + "," +  std::to_string(K_in) + "," + std::to_string(N_in));
    }
